...
export function getUserList (queryParams) {
  if (queryParams.username) {
    return http.get(host + '/user?_page=' + queryParams.pageNo + '&username=' + queryParams.username + '&_sort=id&_order=DESC')
  } else {
    return http.get(host + '/user?_page=' + queryParams.pageNo + '&_sort=id&_order=DESC')
  }
}

export function getUserListCount (queryParams) {
  if (queryParams.username) {
    return http.get(host + '/user?username=' + queryParams.username)
  } else {
